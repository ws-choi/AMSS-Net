...
class pocm_control_model(nn.Module):
    def __init__(self, dense_control_block, n_blocks, internal_channels, gamma_activation=nn.Identity,
        ...
        self.num_target_block = num_target_block
    ...
    def forward(self, x):
        ...
        gammas, betas = self.gen_params(x, self.linear_gamma, self.linear_beta)
        return gammas, betas

    def gen_params(self, x, linear_gamma, linear_beta):
        m = self.n_blocks // 2
        if self.full:
            gammas = self.gamma_split(linear_gamma(x), 0, self.n_blocks)
            betas = self.beta_split(linear_beta(x), 0, self.n_blocks)
            ...
            gammas = self.gamma_split(linear_gamma(x), 0, m)
            betas = self.beta_split(linear_beta(x), 0, m)
...
class gpocm_control_model(pocm_control_model):

    def __init__(self, dense_control_block, n_blocks, internal_channels, gamma_activation=nn.Identity,
                 beta_activation=nn.Identity, pocm_to='full', pocm_norm=None):
        super().__init__(dense_control_block, n_blocks, internal_channels, gamma_activation, beta_activation, pocm_to,
                         pocm_norm)
        ...
        def mk_norm(pocm_norm, type):
            if pocm_norm is None:
                ...
                if type == 'gamma':
                    return nn.BatchNorm1d(self.num_target_block * (internal_channels ** 2), affine=False)
                elif type == 'beta':
                    return nn.BatchNorm1d(self.num_target_block * internal_channels, affine=False)
            elif 'lstm_like' == pocm_norm:
                if type == 'gamma':
                    return nn.BatchNorm1d(self.num_target_block * (internal_channels ** 2), affine=False)
                elif type == 'beta':
        ...
        self.linear_gamma2 = nn.Sequential(
            nn.Linear(dense_control_block.last_dim, self.num_target_block * (internal_channels ** 2)),
            self.gamma_activation,
            mk_norm(pocm_norm, 'gamma')
        )
        self.linear_beta2 = nn.Sequential(
            nn.Linear(dense_control_block.last_dim, self.num_target_block * internal_channels),
            self.beta_activation,
            mk_norm(pocm_norm, 'beta')
        )
        self.linear_gamma3 = nn.Sequential(
            nn.Linear(dense_control_block.last_dim, self.num_target_block * (internal_channels ** 2)),
            self.gamma_activation,
            mk_norm(pocm_norm, 'gamma')
        )
        self.linear_beta3 = nn.Sequential(
            nn.Linear(dense_control_block.last_dim, self.num_target_block * internal_channels),
            self.beta_activation,
            mk_norm(pocm_norm, 'beta')
        )
    ...
    def forward(self, x):
        x = self.dense_control_block(x)
        gammas1, betas1 = self.gen_params(x, self.linear_gamma, self.linear_beta)
        gammas2, betas2 = self.gen_params(x, self.linear_gamma2, self.linear_beta2)
        gammas3, betas3 = self.gen_params(x, self.linear_gamma3, self.linear_beta3)
        return gammas1, betas1, gammas2, betas2, gammas3, betas3
