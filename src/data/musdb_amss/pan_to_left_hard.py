...
class Musdb_Pan2Left_Hard(Selective_Audio_Editing):
    ...
    def __init__(self, target_names):
        ...
        self.descriptions = ['pan {} completely to the left side'.format(', '.join(d)) for d in target_permute]
    ...
    def edit(self, unmixed_track):
        ...
        for idx in self.targets_index:
            mean = manipulated_track[idx].mean(axis=-1)
            manipulated_track[idx, :, 0] = mean * 1.0
            manipulated_track[idx, :, 1] = mean * 0.0
    ...
    def edit_for_test(self, unmixed_track):
        ...
        for idx in range(4):
            if idx in self.targets_index:
                track = manipulated_not4return[idx]
                tar_before = tar_before + track
                mean = track.mean(axis=-1)
                manipulated_not4return[idx, :, 0] = mean * 1.0
                manipulated_not4return[idx, :, 1] = mean * 0.0
                tar_after = tar_after + manipulated_not4return[idx]
            else:
                acc = acc + manipulated_not4return[idx]

        linear_sum = np.sum(unmixed_track, axis=0)
        # manipulated = np.sum(manipulated, axis=0)
...
def amss_pan2left_hard_generator():
    amss_pan2left_set = []
    for i in [1, 2, 3]:
        for comb in combinations(targets, i):
            amss_pan2left_set.append(Musdb_Pan2Left_Hard(comb))

    return amss_pan2left_set
