...
class Musdb_Increase_Volume(Selective_Audio_Editing):
    ...
    def __init__(self, target_names):
        ...
        self.descriptions = ['decrease the volume of {}'.format(', '.join(d)) for d in target_permute]
...
def amss_decrease_generator():
    amss_decrease_set = []
    for i in [1, 2, 3]:
        for comb in combinations(targets, i):
            amss_decrease_set.append(Musdb_Increase_Volume(comb))

    return amss_decrease_set
